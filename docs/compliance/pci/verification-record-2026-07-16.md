...
| Current verified test inventory | Frontend 14 files/62 tests, API 47 files/258 tests, and scripts 18 suites/76 tests all passed against the current integrated worktree: 396 tests total; frontend, API, and scripts TypeScript checks passed, and the frontend production build passed |
...
| Requirement 6.5 adoption record | Added a tool-neutral decision record for exact procedure identity, authority/delegates, system-of-record controls, population sources, reconciliation cadence, retention, 6.2.3 non-author review, normal/emergency rules, approval, and initial evidence milestones. Updated the decision pack and role evidence to remove current GitHub prerequisites. The template is unfilled and does not prove adoption or operation |
| Pull-request change-record gate | Seven tests passed for a complete normal record, placeholder/CDE/section rejection, author self-review and significant-change rejection, emergency authority/deadline enforcement, repository-template field/section coverage, overlapping-comment removal, and an honest pending/unassigned PR record. Hosted PR mode allows only the pending/unassigned combination with a specific rationale; strict invocation still requires approval and a real distinct reviewer. |
...
| Public security-reporting gate | Four portable tests passed for exact GitHub private-advisory and policy discovery through RFC 9116, removal of the broken `/security/report/` link from overview and sitemap, PCI Readiness discovery, sensitive-data and safe-testing boundaries, absence of invented email/SLA claims, explicit assurance limits, and a current annual expiry. Public checks observed the intended `/security/` overview and RFC 9116 record; private-intake enablement/monitoring, approved owner identity, and synthetic closure remain unverified. |
...
| Security-readiness continuity ledger | HTML parsed successfully with 11 unique IDs; required feedback, continuity, verification, blocker, and workstream markers were present; all 94 links/anchors resolved: 11 internal, 82 local artifacts, and one official external source |
...
| Public security HTML and reporting target | Python standard-library HTML parser completed without error; all 19 evidence grades use the approved vocabulary and include an adjacent evidence-scope label. Public overview links, reporting actions, repository policy, RFC 9116 contact, README workflow badge, and portable tests now target `ryanportfolio/Truenote`; current workflow wording requires hosted evidence without treating branch enforcement as the PCI Requirement 6.5 prerequisite |
