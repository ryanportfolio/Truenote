...
| Login archive plate | 1.8s document-alignment sequence, then a 14s masked depth cycle with one mineral-edge refraction and a long rest; all disabled by `prefers-reduced-motion` and low-performance mode |
...
1. **Login hero** — `public/visuals/luminous-archive-diagonal.webp` (1920×1080 WebP q84, ~174KB), an original editorial archive sculpture generated for Truenote. Irregular paper strata, cobalt indexing planes, one translucent mineral sheet, and a persimmon tab gather around a recessed evidence seam. Four feathered copies of the same image follow those physical depth planes: documents align over 1.8 seconds, then drift in opposition during a restrained 14-second cycle while one light passes along the mineral edge and catches the tab. There are no orbital paths, radial glows, particles, or full-panel sweeps. Reduced-motion and low-performance modes render the complete base image as a still. The sign-in panel carries the **BrandField** watercolor at near-full strength (opacity 0.85, bottom-fade mask — see below) so the living field and the sculpture share the surface. Demo deployments extend the archive metaphor into two large role portals: mineral sheets and orbit lines frame a concise role promise while the selected portal tightens around its cobalt core.
