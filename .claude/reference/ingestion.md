...
7. **Embed** — `text-embedding-3-small` for each clean chunk. Insert into `chunks` with `content`, `embedding`, and denormalized `program_id`; PostgreSQL generates `content_tsv` from `content`.
