...
- **Generation path**: per-question `generationPath` plus `fallbackGenerationCount` / `failedFallbackCount` show when the configured primary route degraded to the direct OpenAI fallback or both routes failed.
