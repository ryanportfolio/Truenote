...
- **Bulk CSV user import creates CSR accounts only.** Emails are normalized and deduplicated, existing accounts are skipped without mutation, and the effective program is enforced server-side. Each new user receives a per-user cryptographically random temporary password with `must_reset_password=true`; only hashes are stored, and the generated plaintext passwords are returned once in the import response for out-of-band delivery.
