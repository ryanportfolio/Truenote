...
- **`app_settings` never authorizes arbitrary model ids or providers.** The model-routing API accepts only ids from the server-owned allowlist; the JSONB row stores the selected preset id, not an executable request body. Missing table/row/invalid value falls back to GPT-5.6 Luna on OpenAI (low reasoning).
